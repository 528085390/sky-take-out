...
public class WorkSpaceServiceImpl implements WorkSpaceService {
    ...
    public BusinessDataVO getBusinessData() {
        ...
        // 营业额
        List<OrderStatisticsDTO> orderStatisticsDTO = orderMapper.getTurnoverByDateRange(today, today, Orders.COMPLETED);
        BigDecimal turnover = orderStatisticsDTO.stream().map(OrderStatisticsDTO::getTurnover).reduce(BigDecimal.ZERO, BigDecimal::add);

        // 有效订单数 订单完成率
        Integer totalOrderCount = orderMapper.countByDate(null, null, today, null);
        Integer validOrderCount = orderMapper.countByDate(null, null, today, Orders.COMPLETED);
        ...
        // 新增用户数
        Integer newUsers = userMapper.countByDate(null, null, today);
